    ...
    adminAndCommandEvent
        ...
        .flatMap(
            channel -> {
              ...
              if (constants.numberOfPlayers < constants.MIN_PLAYERS
                  || constants.numberOfPlayers > constants.MAX_PLAYERS) {
                return channel.createMessage(
                    "Set number of players within range "
    ...
    for (String cardName : constants.CARD_NAMES) {
      ...
      Card card =
          new Card(
              ...
              adminAndCommandEvent.subscribe());
    ...
    messageFlux
        ...
        .flatMap(
            messageChannel -> {
              ...
              if (addCards(names)) {
                message =
                    constants.unusedCards.size() == 0
                        ? "Added cards to deck. All unused cards added to playing deck"
                        : "Added cards to deck.";
              } else {
                message = "Couldn't find cards";
              }
              messageCreateSpecConsumer =
                  messageCreateSpec -> messageCreateSpec.setContent(message);
        ...
        .flatMap(
            messageChannel -> {
              ...
              if (removeCard(names)) {
                message =
                    constants.unusedCards.size() == 0
                        ? "Removed cards from deck. All playing cards removed from playing deck"
        ...
        .flatMap(
            messageChannel -> {
              messageCreateSpecConsumer =
                  messageCreateSpec ->
                      messageCreateSpec
                          .setContent(card.getCardName())
                          .addFile(card.getCardName() + ".png", card.getInputStream());
